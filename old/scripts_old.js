window.addEventListener('load', function() {
  ...
  mapImg.addEventListener('click', function(event) {
    ...
    var x = event.clientX - rect.left - 7.5; //x position within the element.
    var y = event.clientY - rect.top - 7.5;  //y position within the element.
  ...
  function zoomin(element, percent, error) {
    ...
    var currHeight = element.getBoundingClientRect().height;
    if (currWidth >= 12000) return false;
    else {
  ...
  function fit(element, error) {
    ...
    if (currWidth < 740 && currWidth > 750) {
      return ;
    } else if (currWidth > windowWidth) {
      var j = 749.424 / currWidth;
      zoomout(element, j, error);

    } else if (currWidth < windowWidth) {
      var k = currWidth / 749.424;
      zoomin(element, k, error);
      var newWidth = element.getBoundingClientRect().width;
      var newHeight = element.getBoundingClientRect().height;
      dragme.style.left = ((windowWidth - parseFloat(newWidth))/2) + 'px';
      dragme.style.top = ((windowHeight - parseFloat(newHeight))/3) + 'px';
    } else {
      return;
    }
  }
